...
document.addEventListener('DOMContentLoaded', function() {
    const heading = document.querySelector('.intro-section h1');
    const titles = ["Web Designer", "Data Analyst"];
    let titleIndex = 0;
    let currentIndex = 0;
    ...
    function animateText() {
        ...
        const displayText = "I am " + currentTitle.substring(0, currentIndex);
        ...
        if (!isDeleting && currentIndex < currentTitle.length) {
            // Typing forward
            currentIndex++;
            typingSpeed = 150 + Math.random() * 50; // Natural typing variation
            ...
            typingSpeed = isDeleting ? 1500 : 500; // Pause at ends
        }
...
// internship page started
document.querySelectorAll('.view-details-btn').forEach(button => {
    button.addEventListener('click', () => {
        // alert('More details coming soon!');
        redirect: window.location.href = 'details.html';
    });
});
// internship.js
document.querySelectorAll('.view-details-btn').forEach((button, index) => {
    button.addEventListener('click', () => {
        const internshipId = index; // 0, 1, 2, etc. for now
...
if (!popup.querySelector('.verification-link')) popup.appendChild(popupVerificationLink); // Only append if not already present

let hoverTimeout;

// Show popup after 2 seconds of hovering
certificateCards.forEach(card => {
    ...
    card.addEventListener('mouseleave', () => {
        clearTimeout(hoverTimeout); // Cancel if pointer leaves before 2s
    });
});

// Hide popup only when user leaves popup itself
popup.addEventListener('mouseleave', () => {
    popup.classList.remove('active');
    popupOverlay.classList.remove('active');
});

// Clicking the overlay also closes the popup
